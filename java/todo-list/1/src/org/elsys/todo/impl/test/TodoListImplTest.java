...
public class TodoListImplTest {
	...
	public void filterByAndCriteria() {
		assertEquals(0, todoList
				...
				.and(Criteria.priority(Priority.NORMAL))).getTasks().size());
		assertEquals(3, todoList
				.filter(Criteria.priority(Priority.LOW)
	...
	public void filterByOrCriteria() {
		assertEquals(5, todoList
				.filter(Criteria.priority(Priority.LOW)
				.or(Criteria.priority(Priority.NORMAL))).getTasks().size());
		assertEquals(3, todoList
				.filter(Criteria.priority(Priority.LOW)
				.and(Criteria.status(Status.TODO))).getTasks().size());
	}
	...
	@Test
	public void filterByTagsCriteria() {
		String[] tags = new String[2];
		tags[0] = "health";
		tags[1] = "social";
		assertEquals(2, todoList
				.filter(Criteria.tags(tags)).getTasks().size());
		assertEquals(3, completedTodoList
				.filter(Criteria.tags(tags)).getTasks().size());
	}
